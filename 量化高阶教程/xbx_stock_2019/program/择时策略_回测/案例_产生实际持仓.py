...
# ===导入数据
df = pd.read_hdf(r'C:\Users\sgwat\Desktop\quant_class\LEO_DIY\data\maotai_signals.h5', key='df')


# ===由signal计算出实际的每天持有仓位
# 在产生signal的k线结束的时候，进行买入
df['signal'].fillna(method='ffill', inplace=True) # 取最近的一个非空值补全所有的空值
df['signal'].fillna(value=0, inplace=True)  # 将初始行数的signal补全为0
df['pos'] = df['signal'].shift() # 向下移动一行
df['pos'].fillna(value=0, inplace=True)  # 将初始行数的pos补全为0

# ===对涨跌停无法买卖做出相关处理。
# 找出收盘价无法买入的K线
cannot_buy_condition = df['收盘价'] >= df['涨停价']
# 将找出上一周期无法买入的K线、并且signal为1时，的'pos'设置为空值, 补充说明： 上一周期发出买入信号但是涨停，所以position维持不变
df.loc[cannot_buy_condition.shift() & (df['signal'].shift() == 1), 'pos'] = None  # 2010-12-22

# 找出收盘价无法卖出的K线
cannot_sell_condition = df['收盘价'] <= df['跌停价']
# 将找出上一周期无法卖出的K线、并且signal为0时的'pos'设置为空值, 补充说明： 上一周期发出卖出信号但是跌停，所以position维持不变
...
print(df[['交易日期', '收盘价', '涨停价', '跌停价', 'signal', 'pos']].tail(200))
