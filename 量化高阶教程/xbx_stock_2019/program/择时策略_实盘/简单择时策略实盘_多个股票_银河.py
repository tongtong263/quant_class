...
def main(stock_df):
    ...
    # ===更新账户信息
    balance_dict, stock_df = update_account_data(user, stock_df)
    ...
    if sell_stock_list and if_trade:
        ...
        for stock_code in sell_stock_list:
            ...
            # amount = 100
            order_info = user.sell(user, code=stock_code[2:], price=order_price, amount=amount)
    ...
    if buy_stock_list and if_trade:
        ...
        for stock_code in buy_stock_list:
            ...
            if buy_amount >= 100:
                # 下单
                order_info = user.buy(user, code=stock_code[2:], price=order_price, amount=buy_amount)
